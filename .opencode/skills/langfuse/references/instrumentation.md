...
Application → SDK (buffered) → Background exporter → Langfuse backend → Dashboard
                                                        ↑
                                               Processing queue (1-2 min)
```

This is **normal behavior**, not a bug. The SDK sends traces immediately, but they enter a processing queue on the server side.

**If traces don't appear immediately:**
1. Wait 1-2 minutes and check again
2. Use the API to verify traces are being received:

```bash
curl -u "pk-lf-xxx:sk-lf-xxx" "https://cloud.langfuse.com/api/public/traces?limit=1"
```

If the API returns traces, they're working - just waiting for async processing.
